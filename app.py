...
def change_username(session_id, device_id, iid, last_username, new_username):
    ...
    sig = run(parm, md5(data.encode("utf-8")).hexdigest(), session_id)
    headers = {
    ...
    response = requests.post("https://api.tiktokv.com/passport/change/username/v1/", data=data, headers=headers, cookies={"sessionid": session_id})
    return "Success" if response.ok else "Failed"
...
def change_username_route():
    try:
        ...
        iid = "your_iid"
        last_username = get_profile(session_id, device_id, iid) # This fetches the current username
